...
gamTrans <- function(coords,
  ...
  cov_terms_string <- paste(sprintf(
    "s(%s, %s)",
    cov_names,
    parseArgsS(s_args)
  ),
  ...
  if (cond) {
    cond_terms_string <- paste(sprintf(
      "s(%s, %s, by = condition)",
      cond_names,
      parseArgsS(s_args)
    ),
  ...
  if (predict) {
    ...
    if (cond) {
      ...
      # set new raster values
      trans_cond_ras <- insertRaster(
        raster = condition_covs,
        new_vals = vals_trans,
        idx = cell_idx
      )
    } else { # if not cond
