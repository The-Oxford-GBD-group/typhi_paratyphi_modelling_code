...
## TODO temp fix to get biwavelet working
.libPaths(c(.libPaths(), "/ihme/homes/miker985/scratch-space/R-sing-temp-lib-loc"))
# installed via the following command on 2019-11-12 against share/singularity-images/lbd/lbd_rpkgs3.5.0_2_gcc7mklrstudioserver1.1.453.simg
...
## run_date          <- "2019_05_15_full_OOS"

## indicator_group   <- 'training'
## indicator         <- 'tr_had_diarrhea'
## run_date          <- '2020_02_06_11_36_05'

sharedir <- paste(
  '/share/geospatial/mbg', indicator_group, indicator, sep = '/')
commondir <- paste(
  core_repo, 'mbg_central/share_scripts/common_inputs', sep = '/')
package_list <- c(
  t(read.csv(paste(commondir, 'package_list.csv', sep = '/'), header = FALSE)))
...
## plotList <- readRDS(paste0(
##   "/share/geospatial/mbg/",
##   sprintf("%s/%s/output/%s/", indicator_group, indicator, run_date),
##   "vizualization_outputs/plotList.RDS"))
