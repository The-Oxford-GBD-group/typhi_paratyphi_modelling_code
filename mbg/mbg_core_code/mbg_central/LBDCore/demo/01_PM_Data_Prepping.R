...
modeling_shapefile_version <- DAG_obj$pipeline$config_list$modeling_shapefile_version
year_list <- DAG_obj$pipeline$config_list$year_list
seed <- DAG_obj$pipeline$config_list$seed
...
if (holdout != 0) {
  ...
  df <- process_input_data(df, 
                           pop_release                = DAG_obj$pipeline$config_list$pop_release, 
                           interval_mo                = DAG_obj$pipeline$config_list$interval_mo, 
                           modeling_shapefile_version = DAG_obj$pipeline$config_list$modeling_shapefile_version,
                           poly_ag                    = as.logical(DAG_obj$pipeline$config_list$poly_ag))
}

if(as.logical(DAG_obj$pipeline$config_list$use_subnat_res)) {
  
  #get adm0 codes for countries to get subnat REs
  if("all" %in% DAG_obj$pipeline$config_list$subnat_country_to_get){
    countries_to_get_subnat_res <- get_adm0_codes(reg, shapefile_version = DAG_obj$pipeline$config_list$modeling_shapefile_version)
  } else {
    countries_to_get_subnat_res <- get_adm0_codes(DAG_obj$pipeline$config_list$subnat_country_to_get, shapefile_version = DAG_obj$pipeline$config_list$modeling_shapefile_version)[get_adm0_codes(DAG_obj$pipeline$config_list$subnat_country_to_get, shapefile_version = DAG_obj$pipeline$config_list$modeling_shapefile_version) %in% get_adm0_codes(reg, shapefile_version = DAG_obj$pipeline$config_list$modeling_shapefile_version)]
  }
  
  #load and subset standard admin1 shape to countries with subnational REs 
  subnat_full_shp    <- readRDS(get_admin_shapefile( admin_level = 1, raking = F, suffix = '.rds', version = DAG_obj$pipeline$config_list$modeling_shapefile_version ))
...
if (as.logical(DAG_obj$pipeline$config_list$test)) {
...
pipeline_postamble(headnode = TRUE, addl_objs_to_save = "seed")
