...
from stgpr_helpers.api.constants import columns
...
# give st jobs a chance to finish saving
time.sleep(15)
...
class Intermediaries:
    ...
    def enumerate_parameters(self):
        ...
        """if empty, set amp_cutoff to 80th percentile
        of data density - that's just what we're doing here"""
    ...
    def create_nsv_amp_dataframes(self):
        ...
        temp['st_level'] = stgpr_helpers_internal.transform_data(
            temp[self.stage2], self.data_transform, reverse=True
        )
        temp['data_level'] = stgpr_helpers_internal.transform_data(
            temp[self.datavar], self.data_transform, reverse=True
        )
        temp['variance_level'] = stgpr_helpers_internal.transform_variance(
            temp['data_level'],
    ...
    def add_nsv_to_variance(self):
        ...
        if self.nsv_on:
            ...
            tmp['data'] = stgpr_helpers_internal.transform_data(
                tmp['data'], self.data_transform, reverse=True
            )
            tmp['variance'] = stgpr_helpers_internal.transform_variance(
    ...
    def calculate_prod_amplitude(self):
        """Not excited that this needs to exist. Basically, there's
        an amplitude calculation method I forgot to add that almost
        no one uses anymore because few people use old (prod) ST-GPR,
        but some do and decomp is a thing.
    ...
    def calculate_amplitude(self):
        ...
        if self.amp_method in ['global_above_cutoff', 'broken_stick']:
            ...
            # UGH but if amp_cutoff == 0 then you get repeat cols, this is dumb
            ...
            for sex in self.prediction_sex_ids:
                ...
                if (not tmp.empty) & (len(tmp.adj_amp.unique()) > 1):
                    ...
                    if self.amp_method == 'broken_stick':
                        ...
                        print(("Dude - there were NO locations below your cutoff."
