  ...
  # FQ_Typhi <- read.csv('/ihme/covariates/ubcov/model/output/174260/model_estimates.csv')
  # 
  # transform covs
  ...
  if('gam' %in% child_models){
    ...
    # gam_formula <- paste0('response ~ 1+ s(', paste(covs_to_include[covs_to_include!='QA'], collapse = ", bs = 'ts', k = 3) + s("), ", bs = 'ts', k = 3)")
  ...
  if('enet' %in% child_models | 'ridge' %in% child_models | 'lasso' %in% child_models){
    ...
    if('ridge' %in% child_models){mydata[,'ridge_full_pred' := predict(full_ridge,newx = vars, s = 0.05, type = 'response')]}
    ...
    for(i in 1:5){
      ...
      if('ridge' %in% child_models){baby_ridge = glmnet(x = vars , y= response, family = family, lambda = 0.05, alpha = 0, weights = mydata$w[mydata$fold_id!=i])}
      ...
      #fill in the data
      if('ridge' %in% child_models){mydata[fold_id==i,'ridge_cv_pred' := predict(baby_ridge,newx = new_vars, s = 0.05, type = 'response')]}
    ...
    if('ridge' %in% child_models){covs[,'ridge' := predict(full_ridge,newx = new_covs[,rownames(full_ridge$beta)], s = 0.05, type = 'response')]}
  ...
  # if(region == 'SEA'){
  #   locs <- locs[locs$spr_reg_id == 4,]
  # } else if(region == 'SA'){
  #   locs <- locs[locs$spr_reg_id == 158,]
  # }
  locs <- locs[locs$spr_reg_id == 158 |locs$spr_reg_id == 4,]
  
  covs <- data.table(covs)
  # covs[, gam := inv.logit(gam)]
  # covs[, enet := inv.logit(enet)]
  # covs[, ridge := inv.logit(enet)]
