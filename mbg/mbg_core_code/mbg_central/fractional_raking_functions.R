...
build_link_table <- function(shapefile_version,
  ...
  if (is.null(custom_shapefile_path)) {
    ...
    if (is.character(polys[[custom_shapefile_field]])) {
      polys[[custom_shapefile_field]] <- as.factor(polys[[custom_shapefile_field]])
    }
  }
  
  # get simple_raster (replaces build_simple_raster_pop)
  master_pop <- raster::brick('/snfs1/WORK/11_geospatial/01_covariates/09_MBG_covariates/WorldPop_total_global_stack.tif')
  ...
  if (!is.null(custom_shapefile_path)) {
    if (is.factor(link_table[[custom_shapefile_field]])) {
      link_table[[custom_shapefile_field]] <- as.character(link_table[[custom_shapefile_field]])
    }
  }
...
#' @examples
#' \dontrun{
#' link <- build_link_table(idpoly, region, cores)
#' }
...
#'
#' @details 
#' addendum by John VanderHeide (jdv6@uw.edu)
#' Oct 11, 2018
