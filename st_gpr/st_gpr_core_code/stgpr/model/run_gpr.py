...
class RunGPR:
    ...
    def clean_gpr(self):
        ...
        for var in gpr_cols:
            level_space_gpr[var] = stgpr_helpers_internal.transform_data(
    ...
    def validate_gpr(self):
        ...
        msg = ("Dude, you've got missings in GP outputs, which is baaad.\n"
               " {mi}\n"
               "See the problematic piece of dataframe above. "
               "Probably a datapoint with waaaay tiny variance screwing "
               "things up, so probs check you data upload "
               "in that spot.").format(mi=mi.to_string())
        assert mi.empty, ValueError(msg)

    def save_gpr_outputs(self):
        """This is a horrifying cascade of if statements..
        Basically, save to different locations based on
        ...
        It's nightmarish but hella efficient compared
        to just loading everything together and spitting it back out."""
