# Jon Mosser <- Roy Burstein <- Rebecca Stubbs
# 09/19/2017 <- 08/28/2017 <- 06/13/2017
# Make Admin 1 and 2 level summary draw-level objects
# Based on stubbs code: https://github.com/RebeccaStubbs/mbg/blob/clean_develop/mbg_central/results/results_aggregate_1.R
...
if (length(commandArgs()) >= 14) {
  ...
  source("/share/code/geospatial/lbd_core/mbg_central/misc_functions.R")
...
if(!(length(pixel_id)==nrow(cell_pred)/length(year_list))){
  stop(sprintf("Excuse me, but the number of valid pixels in your simple raster (%i) is not the same number of valid pixels in your cell_pred object (%i / %i = %i). Look into this!",
               length(pixel_id), nrow(cell_pred), length(year_list), nrow(cell_pred) / length(year_list)))
}else{
