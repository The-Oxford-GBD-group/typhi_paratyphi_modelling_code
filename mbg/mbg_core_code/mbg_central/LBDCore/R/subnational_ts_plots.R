...
subnational_ts_plots <- function(ad0_df,
  ...
  if (is.null(ad0_shape)) {
    ad0_shape_simple <- readOGR(get_admin_shapefile(admin_level = 0, raking = F, version = shapefile_version)) %>%
  ...
  if (is.null(ad1_shape)) {
    ad1_shape_simple <- readOGR(get_admin_shapefile(admin_level = 1, raking = F, version = shapefile_version)) %>%
      subset(ADM0_CODE %in% subset_codes) %>%
      simplify_spdf(tol = tol)
  } else {
  ...
  if (is.null(ad2_shape)) {
    ad2_shape_simple <- readOGR(get_admin_shapefile(admin_level = 2, raking = F, version = shapefile_version)) %>%
      subset(ADM0_CODE %in% subset_codes) %>%
      simplify_spdf(tol = tol)
  } else {
