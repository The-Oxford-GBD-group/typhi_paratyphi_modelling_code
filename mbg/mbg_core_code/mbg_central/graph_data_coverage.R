...
#' @param endemic_gauls This argument can be set as a character
#'   vector containing ISO3 codes. if non-null, the maps will be gray except
#'   for these countries
...
make_background_map <- function(region,
  ...
  if (!is.null(endemic_gauls)) {
    endemic_codes <- get_adm0_codes(endemic_gauls)
    background_map_sub <- background_map[!(background_map$GAUL_CODE %in% endemic_codes), ]
    background_map_df <- suppressMessages(fortify(background_map_sub))
    background_map_df <- as.data.table(background_map_df)
  } else if(stage_3_gray) {
    background_map_sub <- background_map[(background_map$GAUL_CODE %in% stage3$gadm_geoid), ]
    background_map_df <- suppressMessages(fortify(background_map_sub))
    background_map_df <- as.data.table(background_map_df)
  } else {
    background_map_df <- data.table()
  }
  ...
  #if areas to gray out are specified and exist for region in question
  if((!is.null(endemic_gauls) | stage_3_gray) & nrow(background_map_df) > 0){
    background_map_gg_not_endemic <- geom_polygon(data = background_map_df,
                                                  aes(x = long,
                                                      y = lat,
                                                      group = group),
                                                  fill = "lightgray")
...
#'   options: `classic`, `darker_middle`, `red_blue`, `carto_red_blue`, `vaccines`
#'
...
get_color_list <- function(color_scheme) {
  ...
  } else if (color_scheme == "vaccines"){ 
    color_list <-   c("#4d004b", "#5378b2",  "#dbecf7", "#feefc3", 
                      "#fee191", "#FC8D58", "#b03027", "#711f19")
