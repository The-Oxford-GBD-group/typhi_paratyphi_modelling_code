...
reg <- "south_asia"
age <- 0
run_date <- "sadatnfs_multi_stateREs_2019_02_06_08_57_09"
test <- FALSE
holdout <- 0
saveimage <- TRUE
indicator <- indic <- "stunting_mod_b"
indicator_group = "child_growth_failure"
core_repo <- repo            <- paste0('/share/code/geospatial/', user, '/', 'cgf', '/')
...
invisible(.C("setMKLthreads", as.integer(25)))
invisible(.C("setOMPthreads", as.integer(25)))
fix_raster_tmpdir()
# blas_set_num_threads(25)
# omp_set_num_threads(25)

## cores to use
cores_to_use <- 25 # round(as.numeric(slots)*.5)
...
if(as.logical(skiptoinla) == FALSE){
  ...
  # gaul_list_a0        <- get_adm0_codes(reg, shapefile_version = modeling_shapefile_version)
  # gaul_list_a1        <- get_adm_codes_subnat(gaul_convert('IND'), admin_level = 1, shapefile_version = modeling_shapefile_version)
  # gaul_list <- unique(c(gaul_list_a0, gaul_list_a1))
...
##### RELOADING IS VERY VERY NECESSARY #####
...
input_data <- build_mbg_data_stack(df              = df, # note that merge (if using TMB) will return data in a different (but internally consistent) order, just different than df
                                   ...
                                   use_subnat_res  = TRUE, ##### NEW THING
                                   remove_non_subnats = TRUE, ##### NEW THING
...
pm <- lapply(chunks, function(samp) {
  if(fit_with_tmb == FALSE){
    predict_mbg(res_fit       = model_fit,
                ...
                shapefile_version = modeling_shapefile_version,
                simple_raster_subnats = simple_raster2, ##### NEW THING
                subnat_country_to_get = subnat_country_to_get ##### NEW THING
