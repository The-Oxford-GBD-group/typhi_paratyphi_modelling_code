...
# LBD preamble
core_repo <- file.path('/share/code/geospatial/lbd_core')
commondir <- file.path(core_repo, '/mbg_central/share_scripts/common_inputs')
package_list <- c(t(read.csv(file.path(commondir, 'package_list.csv'), header = FALSE)))
source(file.path(core_repo, '/mbg_central/setup.R'))
