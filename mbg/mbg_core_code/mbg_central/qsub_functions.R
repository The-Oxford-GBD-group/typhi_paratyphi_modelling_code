...
#' @title generate_qsub_command
#'
#' @description Generates a qsub command that should work on any IHME cluster.
#'
...
#'
#' @param terse Whether to add a -terse flag so only the job number is returned from the resulting qsub command. Default = FALSE.
#'
#' @return qsub command as a character.
generate_qsub_command <- function(..., stderr_log, stdout_log, project, resources, job_name, singularity_str, queue, cores = NULL, priority = 0, hold_jid = NULL, terse = FALSE) {
