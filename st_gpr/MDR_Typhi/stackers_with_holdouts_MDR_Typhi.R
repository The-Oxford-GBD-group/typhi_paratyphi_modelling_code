...
model_date <- 'MDR_Typhi_final_random15HO'
region <- 'Asia'
...
#Load data
master_data <- data.table(read.csv('/ihme/homes/annieb6/AMR/typhi_paratyphi/datasets/MDR_typhi_outliered_over5.csv', stringsAsFactors = F))
master_data <- master_data[master_data$is_outlier == 0,]


if(region == 'sSA'){
  master_data <- master_data[master_data$super_region == 'Sub-Saharan Africa',]
  child_models <- c('gam', 'rf', 'nnet')
  }else{
  master_data <- master_data[master_data$super_region != 'Sub-Saharan Africa',]
  child_models <- c('gam', 'rf', 'lasso')
}
...
covs <- covs[covs$year_id >= min_year+2 & covs$year_id <= max_year+2,] #limit to the year +2 as have laged the covariates
...
for(h in 1:10){
  ...
  if('enet' %in% child_models | 'ridge' %in% child_models | 'lasso' %in% child_models){
    ...
    if('ridge' %in% child_models){mydata[,'ridge_full_pred' := predict(full_ridge,newx = vars, s = 0.005, type = 'response')]}
    if('lasso' %in% child_models){mydata[,'lasso_full_pred' := predict(full_lasso,newx = vars, s = 0.005, type = 'response')]}
    if('enet' %in% child_models){mydata[,'enet_full_pred' := predict(full_enet,newx = vars, s = 0.005, type = 'response')]}
  ...
  for(i in 1:length(child_models)){
    for(j in 1:length(child_models)){
      if(i==j){
        ...
        if(cor(mydata[,get(paste0(child_models[i], '_cv_pred'))],mydata[,get(paste0(child_models[j], '_cv_pred'))])^2>0.9){message(paste0(child_models[i],  ' and ', child_models[j], ' correlated, remove one'))}
      }
    }
  }
