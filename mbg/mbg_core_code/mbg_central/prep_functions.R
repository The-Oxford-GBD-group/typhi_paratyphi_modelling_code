...
pull_custom_modeling_regions <- function(custom_regions){
  ...
  ref_reg_list <- list(
    ...
    # custom regions of AMR typhi 
    'amr_na' = 'mar+dza+tun+lby+mrt+mli+ner+tcd+sen+gmb+gnb+bfa+cpv',
    'amr_wssa' = 'gin+sle+lbr+civ+gha+tgo+ben+nga+cmr+stp+gnq+gab+mrt+cog+mli+ner+tcd+sen+gmb+gnb+bfa+cpv',
    'amr_essa' = 'sdn+ssd+caf+cod+eri+dji+eth+som+ken+uga+rwa+bdi+tza+mwi+zmb+zwe+moz+mdg+com+ago',
    'amr_me' = 'egy+pse+lbn+syr+tur+irq+irn+sau+yem+omn+are+qat+bhr+kwt',
    'amr_soas' = 'afg+pak+ind+npl+bgd+lka+btn',
    'amr_seas' = 'chn+mmr+lao+tha+vnm+khm',
    'amr_ocea' = 'phl+idn+png+fji+kir+mhl+slb+ton+vut+wsm+mys',
    'amr_name' = 'egy+pse+lbn+syr+tur+irq+irn+sau+yem+omn+are+qat+bhr+kwt+mar+dza+tun+lby',
    ...
    'lf_endem_afr_ppp_espen' = 'essa+wssa+cssa+sdn+zwe+egy+yem-cpv-ago-nga-uga-ssd-caf-sdn-som-ken-gha',
    'oncho_endem_afr' = 'essa+wssa+cssa+sdn+yem-cpv-zmb-mdg-mrt'
...
plot_custom_regions <- function(region_list,
  ...
  default_reg_table <- data.table(in_s1s2 = T,
                                  gaul_code = get_adm0_codes(c('stage1', 'stage2'), shapefile_version = shapefile_version))
...
#' @author Neal Marquez
#'
#' @param fixed_effects string of fixed effects, from config
#' @param gbd_fixed_effects string of gbd_fixed_effects effects, from config
#' @param fixed_effects_constraints string of fixed_effects_constraints, from config
#' @param gbd_fixed_effects_constraints string of fixed effects, from config
#'
#' @return vector of constraints with names for the covariates
#'
#'@export
covariate_constraint_vectorize <- function(fixed_effects,
                                           gbd_fixed_effects,
                                           fixed_effects_constraints,
                                           gbd_fixed_effects_constraints){
  fixed_effects <-
    unlist(strsplit(unlist(fixed_effects), " \\+ "))
  gbd_fixed_effects <-
    unlist(strsplit(unlist(gbd_fixed_effects), " \\+ "))
  fixed_effects_constraints <-
    eval(parse(text = unlist(fixed_effects_constraints)))
  gbd_fixed_effects_constraints <-
    eval(parse(text = unlist(gbd_fixed_effects_constraints)))
...
#' @param comment_long Optional comment to add to config
#'
...
#' @export
#'
upload_config_to_db <- function(config, user, core_repo, indicator_group, indicator, run_date, verbose = FALSE, comment_long = "") {
  ...
  ## Add long comment
  config_binded[, comment_long := paste0(comment_long)]
