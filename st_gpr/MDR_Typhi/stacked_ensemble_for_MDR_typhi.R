  ...
  if('enet' %in% child_models | 'ridge' %in% child_models | 'lasso' %in% child_models){
    ...
    if('ridge' %in% child_models){covs[,'cv_custom_stg1' := predict(full_ridge,newx = new_covs[,rownames(full_ridge$beta)], s = cv_lambda0$lambda.1se, type = 'response')]}
  ...
  covs <- covs[covs$location_id %in% locs$loc_id,]
