    ...
    if (nrow(coords) != 0) {
      ...
      for (ag_age in ag_ages) {
        ...
        for (year_needed in years) {
          ...
          pop_vals <- raster::extract(worldpop_stack, coords_sp, cellnumbers = T)
          ...
          # Ensure there are no duplicate cells because then the population for
          # that cell would be off. This could happen if pixel centroids were
          # extracted at a finer resolution (e.g. 1km x 1km grid) than the pop
          # data is available at.
          unique_cell_counts <- tapply(rep(1, nrow(pop_vals)), pop_vals$cells, sum)
          unique_cell_counts_df <- data.frame(
            cells = names(unique_cell_counts),
            count = unique_cell_counts
          )
          pop_vals <- merge(pop_vals, unique_cell_counts_df, all.x = T)
          pop_vals <- pop_vals[, 2:(ncol(pop_vals) - 1)] / pop_vals$count
