...
#' @title Extract Covariates
#' @description Extract covariates at data locations
#' @author Daniel Casey
#' 
#' @details unlike some of the other MBG covariate functions, this extract_covariates function should be able to treat
...
#'
#' @param df  data table. Should have columns named latitude and longitude at a miniumum
#' @param covariate_list a list object of rasters-like formats (e.g. layer, brick, stack) or just one of that raster-like object
#' @param id_col string indicating name of column that has a unique identifier for each row of df
#' @param reconcile_timevarying If true, the function enforces period matching. For example, if evi is in 4 periods, there will be one EVI column
#' where the values of the column are period/time/year specific. Period in this case is usually identified by a .#### at the end of the layer name 
#' (usually within a brick). As of 11/2, this is .1, .2, .3 ,.4 but it should work for something like .1990, .2000, etc. more testing is needed
#' @param period_var string indicating name of column in the passed data frame designating the time period that matches the rasters
#' @param return_only_results return only the covariate columns? (better for cbinding)
#' @param centre_scale should the covariates be centered and scaled?
#' @param period_map a data table with 2 columns that maps the data period (e.g. year) to period id (e.g. identifies which covariate raster)
#'
#' @return original input df with extracted covariates as new columns
#' 
#' @export
#'
extract_covariates = function(df, 
                              ...
                              period_var = NULL, 
                              return_only_results = F, 
                              centre_scale = F, 
                              period_map = NULL){
  #Load library packages. Should be redundant
  
  library('raster')
  library('data.table')
  df = copy(df) #data table does weird scoping things. Do this just in case
  
  #create an id column if unspecified
  if(is.null(id_col)){
    df[,rrrr := 1:nrow(df)]
    id_col = 'rrrr'
  }
...
#' @param indicator name of column in data frame with outcome
#' @param indicator_family model family
#' @param plot.main Logical. NOTE THIS DOES NOT SEEM TO BE USED. REMOVE?
#' 
#' @return model fit
#' 
#' @export
  ...
  if(is.null(mod)){
    ...
    mod <- try(
      dismo::gbm.step(data             = as.data.frame(df),
                      ...
                      family           = indicator_family,
                      site.weights     = df[,get(weight_column)],
                      tree.complexity  = 2, #TODO should this be something like max(round(tc/2), 1)?
                      learning.rate    = lr*.001,
                      bag.fraction     = bf,
                      silent           = T,
                      plot.main = F,
...
#' @title Fetch covariate layer
#' @description Given a raster-like object and a period, returns the appropriate layer, assuming chronological order
#' @param ras raster brick, stack or other raster type
#' @param period  what period is desired. Default: 1
#' @return Raster corresponding to that period
#' @export
...
#' @title Predict Model Raster
#' @description Given a model object and some covariates, predict out the results 
#' in raster form
#' @param model_call The model object you want to create rasters from
#' @param covariate_layers A list of raster-like objects named identically to the models fit on the tabular data
#' @param constants Do any constants need to be fed to the prediction step? Vectors, matrices, to be cbinded to data
#' @param indicator_family model family, Default: 'binomial'
#' @param centre_scale_df df that contains centering and scaling info, Default: NULL
#' @return Raster with predictions
#' @export
#' 
...
#' @title Stack Models using GAM/BAM
#' @description Stack the estimates from child modules using GAM
#' @param df The dataset. It must have the *_cv_pred and *_full_pred columns from the child models
#' @param model_names Prefixes of the models to be stacked
#' @param weight_column in df is there a column that specifies the observation weights? Default NULL
#' @param bam Whether or not bam should be used. Default False.
#' @param spline_args List of arguments for spline
#' @param indicator The indicator of analysis
#' @param indicator_family Model family
#' @param cores Number of cores. Default "auto"
#' @return List with dataset including predictions from stacker and the stacker gam
#' @export
gam_stacker = function(df,
...
#' @description Fit a random forest model
#' @param df The dataset
#' @param covariates A vector of covariate names and/or formula in the style of all_fixed_effects (e.g. cov1 + cov2 + cov3)
#' @param additional_terms Additional terms to add to covariate formula. Default NULL
#' @param ntree Number of trees to grow. This should not be set to too small a number, to ensure
...
fit_rf = function(df, 
  ...
  #random forest doesn't have a binomial or poisson option. Use empirical logit (this also keeps it consistent with other methods that return logit probabilities)
...
#' @title Fit Random Forest Model for Stacking
#' @description Fit a random forest model for stacking
#' @param df The dataset
#' @param model_name What do you want to call this?
#' @param fold_id_col Name of the column iding the fold
#' @param covariates A vector of covariate names and/or formula in the style of all_fixed_effects (e.g. cov1 + cov2 + cov3)
#' @param additional_terms onstants, other covarites. Usually only used for year and other non-raster covariates.
#' @param ntree Number of trees to grow. This should not be set to too small a number, to ensure
#' that every input row gets predicted at least a few times.
#' @param indicator The indicator of analysis
#' @param indicator_family Model family
...
#' @return df with predictions from all data and using holdouts
#' @export
fit_rf_child_model = function(df,
                              model_name = 'rf', 
                              fold_id_col = 'fold_id', 
                              covariates = all_fixed_effects, 
                              additional_terms = NULL, 
...
#' @title Take Empirical Logit
#' @description Take the empirical logit given number of successes out of N trials
#' @param success Number of success
#' @param N Number of trials
#' @param epsilon Value to change empirical probability when number of success = 0 or N
#' @return empirical logit
#' @export
...
run_child_stackers <- function(models,
  ...
  if('rf' %in% models){
    rf_child <- fit_rf_child_model(df = input_data,
                                  model_name = 'rf', 
                                  fold_id_col = 'fold_id', 
                                  covariates = covariates, 
                                  additional_terms = NULL, 
    ...
    # tic("Stacking - rf")
    # #Define the formula
    # form <- as.formula(paste0('rate ~ ', covariates))
    # 
    ...
    #                               search = 'grid')
    # 
    # tunegrid <- expand.grid(.mtry=c(1:length(all_cov_layers)))
    # 
    # # Fit model
    # rf_fit <- train(form,
    #                     data = input_data,
    ...
    # 
    # # Extract out of sample and in sample predictions
    # input_data[, 'rf_cv_pred'   := arrange(rf_fit_final$pred, rowIndex)[,"pred"]]
    # input_data[, 'rf_full_pred' := predict(rf_fit_final, input_data)]
    # toc(log = T)
    # 
    # rf_child <- list(dataset = input_data[, c('rf_cv_pred', 'rf_full_pred')],
    #                  rf = rf_fit_final)
    # 
    # input_data$rf_cv_pred <-  NULL
    # input_data$rf_full_pred <-  NULL
