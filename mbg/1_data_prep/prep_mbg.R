...
mydata <- master.data[,.(row_id, nid = source_id, super_region, region, country = iso3, 
                    adm1, adm2, best_geo, year = mid_year, prop_resisance =(percentage_resistant/100)*sample_size,  N = sample_size, rate = percentage_resistant/100)]
mydata$prop_resistance <- round(mydata$prop_resistance,0)
colnames(mydata)[colnames(mydata) == 'prop_resisance'] <- tolower(indicator)

mydata <- mydata[mydata$nid !=3286,] #Excluding this as the numbers look very odd and is in chinese so cannot double check atm
#remove those with <10 isolates
mydata <- mydata[mydata$N>=10,]
...
#save data 
write.csv(mydata, paste0('/ihme/geospatial/mbg/input_data/', indicator, '2020_07_02.csv'), row.names = F)
