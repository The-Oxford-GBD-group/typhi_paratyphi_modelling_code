...
pm <- lapply(chunks, function(samp) {
  if (DAG_obj$pipeline$config_list$fit_with_tmb == FALSE) {
    predict_mbg(
      ...
      seed = seed,
      no_nugget_predict = DAG_obj$pipeline$config_list$no_nugget_predict,
      ...
      use_timebyctry_res = as.logical(DAG_obj$pipeline$config_list$use_timebyctry_res),
    ...
    predict_mbg_tmb(
      ...
      use_full_interacting_effect = as.logical(DAG_obj$pipeline$config_list$use_gp),
      use_space_only_gp = as.logical(DAG_obj$pipeline$config_list$use_space_only_gp),
      use_time_only_gmrf = as.logical(DAG_obj$pipeline$config_list$use_time_only_gmrf),
      use_age_only_gmrf = as.logical(DAG_obj$pipeline$config_list$use_age_only_gmrf),
      coefs.sum1 = DAG_obj$pipeline$config_list$coefs_sum
    )
  }
})
