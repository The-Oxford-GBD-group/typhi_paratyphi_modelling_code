...
class STGPRJobSwarm:
    def __init__(self, run_id, run_type, holdouts, draws, nparallel,
                 ...
                 modelable_entity_id, output_path, prediction_location_set_version,
        ...
        self.workflow = Workflow(
            ...
            resume=True,
            seconds_until_timeout=48 * 60 * 60
        )

        # set up job lists
        self.prep_job = None
        self.stage1_jobs = {}
        self.stage1_upload_job = None
        self.st_jobs = {}
        self.descanso_jobs = {}
        self.gpr_jobs = {}
        self.post_jobs = {}
    ...
    def prep_parallelization_groups(self):
        ...
        locs = stgpr_helpers_internal.get_stgpr_locations(
    ...
    def create_prep_job(self):
        task = PythonTask(
            script=PREP_SCRIPT,
            args=[self.run_id],
            name=f'prep_{self.run_id}',
            num_cores=1,
            m_mem_free='15G',
            max_attempts=2,
            ...
            hard_limits=True,
            j_resource=True, # elmo needs J drive
        )
    ...
    def create_stage1_jobs(self):
        ...
        for ko in list(range(0, self.holdouts + 1)):
            ...
            task.add_upstream(self.prep_job)
            self.workflow.add_task(task)
            self.stage1_jobs[task.name] = task
    
    def create_stage1_upload_job(self):
        if self.custom_stage1:
            return

        upload_task = PythonTask(
            script=STAGE1_UPLOAD_SCRIPT,
            args=[self.run_id],
            name=f'stage1_upload_{self.run_id}',
            num_cores=1,
            m_mem_free=f'25G',
            max_attempts=2,
            max_runtime_seconds=1800,
        ...
        for task in self.stage1_jobs.values():
            upload_task.add_upstream(task)
        self.workflow.add_task(upload_task)
        self.stage1_upload_job = upload_task
        ...
        for ko in range(0, self.holdouts + 1):
            for param_group in range(0, len(self.param_groups)):
                for loc_group in range(0, self.nparallel):
                    ...
                    if self.custom_stage1:
                        # Stage 1 upload job not in workflow. Use stage 1 model jobs for
                        # dependency.
                        for stage1_task in self.stage1_jobs.values():
                            task.add_upstream(stage1_task)
                    else:
                        # Stage 1 upload job in workflow. Use that for dependency.
                        task.add_upstream(self.stage1_upload_job)
    ...
    def create_cleanup_jobs(self):
        ...
        for ko in list(range(0, self.holdouts + 1)):
            task = PythonTask(
                ...
                args=[self.run_id, self.output_path,
                      self.run_type, ko, self.draws],
