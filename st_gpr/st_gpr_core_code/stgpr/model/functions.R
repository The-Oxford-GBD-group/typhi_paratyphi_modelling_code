...
weight_subnat_standard_locations <- function(df, population_path, gbd_round_id, national_level = 3){
  ...
  #ugly if-statement so that former gbd rounds return the same results, since weighting the included subnationals
  #was not a thing (ick)
  if (gbd_round_id < 6) {
...
run.lm <- function(df, prior_model, predict_re, study_covs = NULL, standard_locations_only = TRUE, holdout_num) {
  ...
  sum.out <- NULL
  for (sex in sex.run) {
    ## Fun fact, including data with weights of zero returns different fixed AND random effects coefs
    ## than dropping data with weights of zero! I'm dropping....arbitrarily. This way seems more clear-cut.
