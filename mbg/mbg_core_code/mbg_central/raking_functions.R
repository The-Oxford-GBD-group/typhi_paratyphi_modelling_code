...
rake_cell_pred <- function(cell_pred,
  ...
  raking_id_raster <- readRDS(file.path(get_admin_shape_dir(raking_shapefile_version), 'lbd_standard_id_raster.rds'))
  new_raster_list <- build_simple_raster_pop(new_subset_shape, field = field, link_table = raking_link_table, id_raster = raking_id_raster)
  ...
  if(length(new_simple_extract) - length(simple_extract) == 1){
    message("one pixel off")
    new_simple_raster <- mask(new_simple_raster, simple_raster)
    new_simple_extract <- raster::extract(new_simple_raster, extent(new_simple_raster))
    new_simple_extract <- na.omit(new_simple_extract)
  }
  ...
  if(length(simple_extract) != length(new_simple_extract)){
    message("more than one pixel off")
    new_simple_raster <- mask(new_simple_raster, simple_raster)
    new_simple_extract <- raster::extract(new_simple_raster, extent(new_simple_raster))
    new_simple_extract <- na.omit(new_simple_extract)
  }
  
  message("length simple extract: ", length(simple_extract))
  message("length new simple extract: ", length(simple_extract))
  message("length cell pred: ", nrow(cell_pred))
...
#' @title Save raking outputs
#' @description Save Outputs from  Raking Function
#'
...
crosswalk_cell_pred_add_NA <- function(simple_raster, new_simple_raster, cell_pred, year_list) {
  ...
  rows_to_add <- na.omit(rows_to_add)
