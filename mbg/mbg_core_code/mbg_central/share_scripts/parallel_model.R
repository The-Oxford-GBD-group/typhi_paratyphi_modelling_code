...
## Any additional covariate prep
#include the antibiotic covariates
if(indicator == 'mdr_typhi'){
  abx_covs <- c('ddd_per_1000')
}else if(indicator == 'fqns_typhi'){
  abx_covs <- c('J01M')
}else{
  abx_covs <- NULL
}
...
#remove some of the covariates which wont cover the whole region (ie modelled ones)
if(reg == 'amr_me' | reg == 'amr_ocea'| reg == 'amr_name+sdn'| reg == 'amr_name'){
  fixed_effects = "gdp + crutstmp + access2 + ghslurbanicity + fgh_the" 
  fixed_effects_measures= "perpixel + mean + mean + mean + mean"
  fixed_effects_constraints = "c(0, 0, 0, 0, 0, 0)"
  fixed_effects_config <- fixed_effects_config[fixed_effects_config$covariate!='dpt3_cov',]
  fixed_effects_config <- fixed_effects_config[fixed_effects_config$covariate!='stunting_mod_b',]
}

if(reg == 'amr_seas' & indicator == 'fqns_typhi'){
  gbd_fixed_effects <- ""
  gbd_fixed_effects_measures <- ""
  gbd_fixed_effects_constraints <- ""
  gbd_fixed_effects_config <- gbd_fixed_effects_config[gbd_fixed_effects_config$covariate != 'hospital_beds_per1000']
  gbd_fixed_effects_config <- gbd_fixed_effects_config[gbd_fixed_effects_config$covariate != 'sanitation_prop']
}

#for wssa, replace hospital beds with sanitation GBD covariate
if(reg == 'amr_wssa' & indicator == 'mdr_typhi'){
  gbd_fixed_effects <- "sanitation_prop"
  gbd_fixed_effects_config$covariate[gbd_fixed_effects_config$covariate == 'hospital_beds_per1000'] <- 'sanitation_prop'
}
...
if(as.logical(use_subnat_res)) {
  ...
  if (nchar(gbd_fixed_effects) > 0) {
    ...
    #for some reason oceania reads it as a Raster stack which messes up things later, change here
    if(class(gbd_cov_layers[[1]]) == 'RasterBrick'){}else{
      gbd_cov_layers[[1]] <- brick(gbd_cov_layers[[1]])
    }
 
    if(length(gbd_fixed_effects_config$covariate) == 2){
      if(class(gbd_cov_layers[[2]]) != 'RasterBrick'){
        gbd_cov_layers[[2]] <- brick(gbd_cov_layers[[2]])
      }
    }
}
  ...
  abx_cov_layers <- load_abx_covariates(covs           = abx_covs, 
                                        year_ids       = year_list, 
                                        template       = cov_layers[[1]][[1]],
                                        simple_polygon = simple_polygon,
                                        input_file     = '/share/homes/annieb6/covariates/cleaned_covs.csv')
  
  #for some reason oceania reads it as a Raster stack which messes up things later, change here
  if(class(abx_cov_layers[[1]]) == 'RasterBrick'){}else{
    abx_cov_layers[[1]] <- brick(abx_cov_layers[[1]])
  }
  if(length(abx_covs) == 2){
    if(class(abx_cov_layers[[2]]) != 'RasterBrick'){
      abx_cov_layers[[2]] <- brick(abx_cov_layers[[2]])
    }
  }
  } 
  ...
  all_cov_layers <- c(cov_layers, gbd_cov_layers, abx_cov_layers)
  ...
  #as the data are all resample polygons ensure you are holding out whole surveys
  # nid <- sample(unique(the_data$nid))
  # nid <- data.table(nid)
  # nid$fold_id <- cut(seq(1,nrow(nid)),breaks=5,labels=FALSE)
  # the_data <-  merge(the_data, nid, by = 'nid', all.x = T)
  ...
  if(as.logical(use_stacking_covs)){
    ...
    # the_data$mdr_typhi <- the_data$mdr_typhi+1
    # Run the child stacker models 
    child_model_run <- run_child_stackers(models = child_model_names, input_data = the_data)
   
    #for some stupid reasons i cannot get this to not add the columns here so remove them!
    # the_data$rf_full_pred <-  NULL
    # the_data$rf_cv_pred <-  NULL
...
mbg_formula <- build_mbg_formula_with_priors(fixed_effects               = all_fixed_effects,
                                             ...
                                             add_nid_res                 = as.logical(use_nid_res),
                                             nid_re_prior                = nid_re_prior,
...
if(exists('simple_raster2') == FALSE){
  simple_raster2 <- NULL
}
