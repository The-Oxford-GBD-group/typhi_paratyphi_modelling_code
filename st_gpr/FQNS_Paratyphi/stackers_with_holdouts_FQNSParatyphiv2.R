...
model_date <- 'FQNS_Paratyphi_final_min20'

outputdir <-  paste0('/ihme/homes/annieb6/AMR/typhi_paratyphi/stackers/holdouts/', model_date, '/')
dir.create(outputdir, showWarnings = F, recursive = T)

#Load data
master_data <- data.table(read.csv('/ihme/homes/annieb6/AMR/typhi_paratyphi/datasets/FQNS_paratyphi.csv', stringsAsFactors = F))
master_data <- master_data[master_data$is_outlier == 0,]
master_data <- master_data[master_data$sample_size>=20,] 
master_data$val[master_data$val>=1] <- 0.999
...
covs_to_include <-  c( "cv_pollution_outdoor_pm25",
                       "cv_diabetes",
                       "cv_water_prop",
...
if(holdout_method == 'random'){
  ...
  master_data[,master_fold_id := cut(seq(1,nrow(master_data)),breaks=10,labels=FALSE)]
}

if(holdout_method == 'country'){
  country <- unique(master_data[, country])
  country <- country[sample(length(country))]
  master_fold_id <- cut(seq(1,length(country)),breaks=10,labels=FALSE)
...
write.csv(master_data, paste0('/ihme/homes/annieb6/AMR/typhi_paratyphi/stackers/holdouts/', model_date, '/master_data.csv'), row.names = F)

#centre scale the covariates if desired
if(centre_scale == TRUE){
  master_data <- data.frame(master_data)
  master_data[colnames(master_data) %in% covs_to_include] <- data.frame(scale(master_data[colnames(master_data) %in% covs_to_include]))
  master_data$year <- master_data$year_id
  master_data$year_id <- scale(master_data$year_id)
  master_data <-  data.table(master_data)
  ...
  covs$year_id <- scale(covs$year_id)
  covs <-  data.table(covs)
}
...
for(h in 1:10){
  ...
  if('enet' %in% child_models | 'ridge' %in% child_models | 'lasso' %in% child_models){
    ...
    for(i in 1:5){
      ...
      if('lasso' %in% child_models){baby_lasso = glmnet(x = vars , y= response, family = family, lambda = cv_lambda1$lambda.min, alpha = 1, weights = mydata$w[mydata$fold_id!=i])}
      if('enet' %in% child_models){baby_enet = glmnet(x = vars , y= response, family = family, lambda = cv_lambda0.5$lambda.min, alpha = 0.5, weights = mydata$w[mydata$fold_id!=i])}
      ...
      if('lasso' %in% child_models){mydata[fold_id==i,'lasso_cv_pred' := predict(baby_lasso,newx = new_vars, s = cv_lambda1$lambda.min, type = 'response')]}
      if('enet' %in% child_models){mydata[fold_id==i,'enet_cv_pred' := predict(baby_enet,newx = new_vars, s = cv_lambda0.5$lambda.min, type = 'response')]}
    }
    ...
    if('ridge' %in% child_models){covs[,'ridge' := predict(full_ridge,newx = new_covs[,rownames(full_ridge$beta)], s = cv_lambda0$lambda.min, type = 'response')]}
    if('enet' %in% child_models){covs[,'enet' := predict(full_enet,newx = new_covs[,rownames(full_enet$beta)], s = cv_lambda0.5$lambda.min, type = 'response')]}
    if('lasso' %in% child_models){covs[,'lasso' := predict(full_lasso,newx = new_covs[,rownames(full_lasso$beta)], s = cv_lambda1$lambda.min, type = 'response')]}
  ...
  if('rf' %in% child_models){
    ...
    # Create model formula
    if(family == 'binomial'){
      form <- as.formula(paste0('logit(p) ~ ', paste(covs_to_include, collapse = " + ")))
    } else if(family == 'gaussian'){
      form <- as.formula(paste0('n ~ ', paste(covs_to_include, collapse = " + ")))
    }
    ...
    tunegrid_final <- expand.grid(.mtry=2)
    ...
    # Extract out of sample and in sample predictions
    mydata[, 'rf_cv_pred'   := inv.logit(arrange(rf_fit_final$pred, rowIndex)[,"pred"])]
    mydata[, 'rf_full_pred' := inv.logit(predict(rf_fit_final, mydata))]
    ...
    #predict out for all locations
    covs[, 'rf' := inv.logit(predict(rf_fit_final, covs))]
  ...
  if('cubist' %in% child_models){
    ...
    # Extract out of sample and in sample predictions
    mydata[, 'cubist_cv_pred'   := inv.logit(arrange(cubist_fit_final$pred, rowIndex)[,"pred"])]
    mydata[, 'cubist_full_pred' := inv.logit(predict(cubist_fit_final, mydata))]
    ...
    #predict out for all locations
    covs[, 'cubist' := inv.logit(predict(cubist_fit_final, covs))]
  ...
  # mydata <- read.csv(paste0(outputdir, 'holdout_2/fitted_child_models.csv'), stringsAsFactors =  F)
  ...
  stg1 <-  covs[, .(location_id, year_id = year,
  ...
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
  # Save input data for each holdout ####
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
  input_data <- master_data[master_data$master_fold_id != h,]
  input_data <- input_data[,.(location_id, year_id = year, nid, country , age_group_id, sex_id, measure_id, val = p, sample_size = d,
                              variance, is_outlier)]
  write.csv(input_data, paste0(outputdir, '/input_data_h', h, '.csv'), row.names = F)
  
}


square <- read.csv('/ihme/homes/annieb6/AMR/typhi_paratyphi/stackers/Final_min20_FQNS_Paratyphi_/square.csv')

for (i in 1:10){
  estimate <- read.csv(paste0('/ihme/homes/annieb6/AMR/typhi_paratyphi/stackers/holdouts/FQNS_Paratyphi_final_min20/holdout_', i, '/custom_stage1_df.csv'))
  mydata <- square[!(square$location_id %in% estimate$location_id),]
  mydata <- rbind(mydata, estimate)
  write.csv(mydata, paste0('/ihme/homes/annieb6/AMR/typhi_paratyphi/stackers/holdouts/FQNS_Paratyphi_final_min20/holdout_', i, '/custom_stage1_df.csv'), row.names = F)
