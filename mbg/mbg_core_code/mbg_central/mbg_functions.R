...
build_mbg_formula_with_priors <- function(fixed_effects,
  ...
  f_nid_re <- as.formula(paste0('~f(nid_re, model = "iid", hyper = list(theta=', nid_re_prior, '))'))
  ...
  if(add_nid_res == TRUE) f_mbg <- f_mbg + f_nid_re
  if(subnat_RE == TRUE) {
...
build_mbg_data_stack <- function(df, 
  ...
  # else do the inla version
    ...
    if(use_nid_res == TRUE){
      #create a 0:n map for nids
      nidz   <- unique(as.numeric(df$nid))
      nid_re_map <- data.table(
        nid    = nidz,
        re_id  = 0:(length(nidz)-1))
      nid_re_vec <- nid_re_map$re_id[match(as.numeric(df$nid),nid_re_map$nid)]
      design_matrix$nid_re <- nid_re_vec
    }
