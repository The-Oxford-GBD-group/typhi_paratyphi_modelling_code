...
load_gbd_data     <- function(gbd_type,
  ...
  loc_lookup <- read.dbf('/snfs1/DATA/SHAPE_FILES/GBD_geographies/master/GBD_2016/master/shapefiles/GBD2016_analysis_final.dbf')
  loc_lookup <-  data.table(loc_lookup)
  gaul_to_loc_id <- loc_lookup[, list(location_id = loc_id, GAUL_CODE)]
  ...
  if (gbd_type == "covariate") {
    ...
    # format and return
    # gbd_estimates <- merge(gbd_estimates, loc_lookup, by="location_id")
    gbd_estimates <- merge(gbd_estimates, gaul_to_loc_id, by="location_id")
    # setnames(gbd_estimates, c('ihme_lc_id', "year_id", "mean_value"), c("name", "year", "mean"))
...
load_cell_preds <- function(indicator_group,
  ...
  if(u5m){
    if(ageasindic==FALSE){
      ...
      load(paste0('/share/geospatial/mbg/',indicator_group,'/',indicator,'_age',agebin,'/output/',rd,'/',
...
get.cov.wts <- function(rd, ## run_date
  ...
  #stackers_used <- stackers_used[c(-2)]
