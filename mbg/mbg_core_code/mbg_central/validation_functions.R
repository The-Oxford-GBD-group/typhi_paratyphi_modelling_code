...
## Required libraries - fields, akima
###################################################################################
...
get_is_oos_draws <- function(ind_gp,
  ...
  all.regions <- all.regions[!is.na(all.regions)]
  # load raw data
  if (!get.oos) {
...
get_pv_table <- function(d,
  ...
  if (plot == TRUE) {
    ...
    for (i in 1:nrow(plot_table)) {
      ...
      if (!is.null(plot_by)) {
        gg <- gg + facet_wrap(~plot_by_column,
                              ncol = plot_ncol
        )
      }
