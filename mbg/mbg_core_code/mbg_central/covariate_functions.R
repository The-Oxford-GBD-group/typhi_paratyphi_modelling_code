...
load_gbd_covariates = function(covs, measures, year_ids, age_ids,
  ...
  fetch_gbd_cov = function(name, measure, afras) {
    ...
    if(2018%in%year_list){
      add_2018 <- gbd[gbd$year == 2017,]
      add_2018$year <- 2018
      gbd <- rbind(gbd, add_2018)
      message('No 2018 values, using 2017')
    }
...
# Load additional covariates not included in GBD or LBD ####
# Input from a specified file

load_abx_covariates = function(covs, 
                               year_ids, 
                               template,
                               simple_polygon, 
  ...
  cov_file <- read.csv(input_file, stringsAsFactors = F)
  
  #restrict to locations in the shapefile
  cov_file <- cov_file[cov_file$location_id %in% world_shape$loc_id,]
  
  # add the gaul code to the covariate file
  loc_ids <- data.frame(world_shape[c('GAUL_CODE', 'loc_id')])
  cov_file <- merge(cov_file, loc_ids, by.x = 'location_id', by.y = 'loc_id')
  
  fetch_abx_cov = function(name, afras) {
    gbd <- cov_file[c('GAUL_CODE', 'year_id', name)]
    names(gbd) <- c('name', 'year', 'mean')
    gbd <-  data.table(gbd)
  ...
  abx_covariates <- lapply(1:length(abx_covs), function(x) fetch_abx_cov(abx_covs[x], afras))
  names(abx_covariates) <- abx_covs
  
  return(abx_covariates)
}
