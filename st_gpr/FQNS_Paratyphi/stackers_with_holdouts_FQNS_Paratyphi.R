...
if(region == 'SEA'){
  master_data <- master_data[master_data$super_region == 'Southeast Asia, East Asia & Oceania',]
  child_models <- c('rf')
} else if(region == 'SA'){
  master_data <- master_data[master_data$super_region == 'South Asia',]
  child_models <- c('ridge','rf', 'gam')
}
...
covs <- covs[covs$year_id >= min_year+2 & covs$year_id <= max_year+2,]
...
for(h in 1:10){
  ...
  if('enet' %in% child_models | 'ridge' %in% child_models | 'lasso' %in% child_models){
    ...
    #use 10 fold CV for this (?? could change to 10 ??)
  ...
  stg1 <-  covs[, .(location_id, year_id = year-2,
